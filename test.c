...
#define CmdConnect       3 // arg2 == MpscRingBuff_t* to connect with
#define CmdConnected     4 // arg2 == MpscRingBuff_t* connected to
...
void send_to_peers(ClientParams* cp) {
  ...
  for (uint32_t i = 0; i < cp->peers_connected; i++) {
    ...
    if (add_non_blocking(&peer->Rbuf, msg)) {
...
static void* client(void* p) {
  ...
  // Init Rbuf
  if (initMpscRingBuff(&cp->Rbuf, cp->msg_count) == NULL) {
    printf(LDR "initMpscRingBuff returned NULL\n", ldr());
    exit(1);
  }

  DPF(LDR "client: param=%p cp->Rbuf=%p\n", ldr(), p, &cp->Rbuf);
  ...
  while (true) {
    ...
    while((msg = rmv(&cp->Rbuf)) != NULL) {
#else
    sched_yield();
    while((msg = rmv_non_stalling(&cp->Rbuf)) != NULL) {
#endif
      if (msg != NULL) {
        cp->cmds_processed += 1;
  ...
  // Flush any messages in the Rbuf
  DPF(LDR "client: param=%p done, flushing rbuf\n", ldr(), p);
...
uint32_t wait_for_rsp(MpscRingBuff_t* rbuf, uint64_t rsp_expected, void* client, uint32_t client_idx) {
  uint32_t retv;
  Msg_t* msg;
  DPF(LDR "wait_for_rsp:+rbuf=%p rsp_expected %lu client[%u]=%p\n",
      ldr(), rbuf, rsp_expected, client_idx, client);

  // TODO: Add MpscRingBuff_t.sem_waiting??
  bool once = false;
  while ((msg = RMV(rbuf)) == NULL) {
    if (!once) {
      once = true;
      DPF(LDR "wait_for_rsp: rbuf=%p waiting for arg1=%lu client[%u]=%p\n",
          ldr(), rbuf, rsp_expected, client_idx, client);
    }
    sched_yield();
  }
  if (msg->arg1 != rsp_expected) {
    DPF(LDR "wait_for_rsp: rbuf=%p ERROR unexpected arg1=%lu expected %lu arg2=%lu, client[%u]=%p\n",
        ldr(), rbuf, msg->arg1, rsp_expected, msg->arg2, client_idx, client);
    retv = 1;
  } else {
    DPF(LDR "wait_for_rsp: rbuf=%p got ar1=%lu arg2=%lu client[%u]=%p\n",
        ldr(), rbuf, rsp_expected, msg->arg2, client_idx, client);
    retv = 0;
  }
  ret_msg(msg);
  DPF(LDR "wait_for_rsp:-rbuf=%p rsp_expected %lu client[%u]=%p retv=%u\n",
      ldr(), rbuf, rsp_expected, client_idx, client, retv);
  return retv;
}
...
bool multi_thread_main(const uint32_t client_count, const uint64_t loops,
  ...
  if (initMpscRingBuff(&Rbuf, msg_count) == NULL) {
    printf(LDR "initMpscRingBuff returned NULL\n", ldr());
    exit(1);
  }
  DPF(LDR "multi_thread_msg: Rbuf=%p\n", ldr(), &Rbuf);
  ...
  for (uint32_t i = 0; i < clients_created; i++) {
    ...
    for (uint32_t peer_idx = 0; peer_idx < clients_created; peer_idx++) {
      ...
      if (peer_idx != i) {
        ...
        if (wait_for_rsp(&Rbuf, CmdConnected, client, i)) {
          error = true;
          goto done;
        }
      }
  ...
  for (uint32_t i = 0; i < clients_created; i++) {
    ...
    add(&client->Rbuf, msg);
    sem_post(&client->sem_waiting);
    if (wait_for_rsp(&Rbuf, CmdDisconnected, client, i)) {
      error = true;
      goto done;
    }
  }
  ...
  for (uint32_t i = 0; i < clients_created; i++) {
    ...
    add(&client->Rbuf, msg);
    sem_post(&client->sem_waiting);
    if (wait_for_rsp(&Rbuf, CmdStopped, client, i)) {
      error = true;
      goto done;
    }
  }
  ...
  // Deinit the Rbuf
  DPF(LDR "multi_thread_msg: deinit Rbuf=%p\n", ldr(), &Rbuf);
  msgs_processed += deinitMpscRingBuff(&Rbuf);
